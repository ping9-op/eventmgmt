      ...
      <div className="g2">
        ...
        <div style={{ background: 'white', border: '0.5px solid var(--border2)', borderRadius: 14, overflow: 'hidden' }}>
          <div style={{ background: 'var(--accent)', padding: '13px 20px', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button onClick={() => navigate('/expo/overview')} style={{ marginLeft: 'auto', background: 'rgba(255,255,255,.2)', border: 'none', color: 'white', padding: '4px 12px', borderRadius: 6, fontSize: 11, cursor: 'pointer', fontWeight: 600, minHeight: 44 }}>{t('view_detail')}</button>
          </div>
          <div style={{ padding: '14px 20px' }}>
            {/* 요약 3칸 */}
            <div className="g3-kpi">
            ...
            {recentExh.map((e, i) => (
              <div key={i} onClick={() => navigate('/expo/overview')} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', borderBottom: '0.5px solid var(--border)', cursor: 'pointer', flexWrap: 'wrap', gap: 4 }}
        ...
        <div style={{ background: 'white', border: '0.5px solid var(--border2)', borderRadius: 14, overflow: 'hidden' }}>
          <div style={{ background: '#1E3A5F', padding: '13px 20px', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button onClick={() => navigate('/sales/dashboard')} style={{ marginLeft: 'auto', background: 'rgba(255,255,255,.2)', border: 'none', color: 'white', padding: '4px 12px', borderRadius: 6, fontSize: 11, cursor: 'pointer', fontWeight: 600, minHeight: 44 }}>{t('view_detail')}</button>
          </div>
          <div style={{ padding: '14px 20px' }}>
            {/* KPI 2×2 */}
            <div className="g2-kpi">
              {[
            ...
            {topLeads.map((l, i) => (
              <div key={l.id} onClick={() => setSelectedLeadId(l.id)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', borderBottom: '0.5px solid var(--border)', cursor: 'pointer', flexWrap: 'wrap', gap: 4 }}
                onMouseOver={ev => (ev.currentTarget as HTMLDivElement).style.opacity = '.7'}
                onMouseOut={ev => (ev.currentTarget as HTMLDivElement).style.opacity = '1'}>
      ...
      <div className="g2-nm">
        ...
        <div style={{ background: 'white', border: '0.5px solid var(--border2)', borderRadius: 14, padding: '18px 22px' }}>
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
            ...
            <button onClick={() => navigate('/expo/schedule')} style={{ background: 'none', border: '1px solid var(--border2)', borderRadius: 6, padding: '3px 10px', fontSize: 11, cursor: 'pointer', color: 'var(--muted)', minHeight: 44 }}>{t('view_all')}</button>
          </div>
          {upcoming.length === 0 ? (
        ...
        <div style={{ background: 'white', border: '0.5px solid var(--border2)', borderRadius: 14, padding: '18px 22px' }}>
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
            ...
            <button onClick={() => navigate('/sales/funnel')} style={{ background: 'none', border: '1px solid var(--border2)', borderRadius: 6, padding: '3px 10px', fontSize: 11, cursor: 'pointer', color: 'var(--muted)', minHeight: 44 }}>{t('view_all')}</button>
          </div>
          {STAGE_ORDER.map(s => {
            ...
            return (
              <div key={s} onClick={() => navigate('/sales/funnel')} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 9, cursor: 'pointer', padding: '3px 6px', borderRadius: 6 }}
                ...
                <div style={{ minWidth: 80, maxWidth: 130, fontSize: 12, color: 'var(--text)', flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s}</div>
